...
| --- | --- | --- |
| `src/app/api/spawn/route.ts` × 2 (`sessions_spawn`) | WS `sessions.create` schema lacks `runTimeoutSeconds`, tools profile, runtime, and cleanup fields that the CLI tool accepts (per Đào msg 1336). Silently mapping would introduce a parity bug. **Classification (PR #20): upstream schema gap / status quo retained; not MC implementation bug.** Source citations + draft upstream issue body live in `docs/audit/PR20_SESSIONS_CREATE_WS_SCHEMA_GAP.md`. The companion `SessionsPatchParamsSchema` also lacks both fields, so the "create + patch shim" route is closed too. | Either extend the WS `sessions.create` schema to cover these fields, OR design a parity shim on the MC server that translates the CLI-style spawn payload into a series of WS calls (`sessions.create` + follow-up `sessions.patch` for the missing fields). |
| ~~`src/app/api/sessions/transcript/gateway/route.ts` × 1 (`chat.history`)~~ | **Migrated via PR #21.** OpenClaw 2026.4.24 SDK exposes `ChatHistoryParamsSchema` at `~/.npm-global/lib/node_modules/openclaw/dist/plugin-sdk/src/gateway/protocol/schema/logs-chat.d.ts:15-18` (`{ sessionKey, limit?, maxChars? }`) — schema-compatible with MC's `{ sessionKey, limit }` payload. The route now calls `callOpenClawGatewayWS('chat.history', ...)`; the disk-side JSONL fallback inside the same route is unchanged. See `docs/audit/PR21_TRANSCRIPT_CHAT_HISTORY_WS_MIGRATION.md`. | — |
| ~~`src/app/api/pipelines/run/route.ts:137` (`runOpenClaw(['agent', '--message', ...])`)~~ | **Migrated via PR #22.** Design call (Đào msg 1576/1580) confirmed the route consumes a single deferred result, not a stream — straight migration to `callOpenClawGatewayWS('agent', …)` with `deliver: true`, deterministic `idempotencyKey`, and the template's `timeout_seconds` forwarded to `params.timeout`. `agentId` resolution falls through to the gateway default unless `OPENCLAW_DEFAULT_AGENT_ID` env is set. See `docs/audit/PR22_PIPELINES_RUN_AGENT_WS_MIGRATION.md`. | — |
