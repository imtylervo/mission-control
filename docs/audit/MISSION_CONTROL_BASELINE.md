...
6. **`mc-device-token` classification — RESOLVED via PR #18 (Phase 1.6).** Classified as **bearer-equivalent** based on local OpenClaw 2026.4.24 gateway source review: `~/.npm-global/lib/node_modules/openclaw/dist/server.impl-CtLS1ywt.js:10538-10579` shows `verifyDeviceToken({ deviceId, token, role, scopes })` runs as a fallback auth path that sets `authMethod="device-token"` without requiring a fresh `device.signature` in the same connect frame. XSS that reads both `mc-device-token` and `mc-device-id` from `localStorage` can replay against the gateway and acquire a session without the private key (which PR #574 already protected via IDB `extractable: false`). Storage-migration implementation tracked as Phase 1.7 in the roadmap. See `docs/audit/PR18_DEVICE_TOKEN_BEARER_CLASSIFICATION.md`.
