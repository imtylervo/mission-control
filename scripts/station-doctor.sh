...
# Diagnostic script — fail-soft. We want every check to run even when an
# earlier one reports a non-zero status. `set -e` combined with bash's
# `((COUNTER++))` post-increment kills the script on the FIRST PASS (the
# expression returns the pre-increment value 0, which errexit treats as
# failure). Use `set -uo pipefail` only and assignment-form increments
# below so counters survive the run.
set -uo pipefail
...
# NB: drop curl -f and the `|| echo "000"` fallback. With -f, curl on a
# connection failure exits non-zero AND -w "%{http_code}" still prints
# "000" on stdout, so the OR fallback would concatenate to "000000". Use
# -s + an OR assignment that overrides only when curl itself fails.
http_code=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:$MC_PORT/api/status?action=health" 2>/dev/null) || http_code="000"
